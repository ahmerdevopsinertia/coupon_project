...
public static function check_coupon_claimed( $uid, $item ) {
    ...
    $stmt->bind_result( $id, $coupon, $user, $code, $user, $user_date, $date );
    $stmt->execute();
    $stmt->fetch();
    $stmt->close();

    if( $id ) {
        return (object) array( 'ID' => $id, 'code' => $code, 'couponID' => $coupon, 'userID' => $user, 'used' => $user, 'used_date' => $user_date, 'date' => $date );
    }
...
public static function submit_store_location( $post ) {
    ...
    if( !isset( $post['store'] ) || !isset( $post['address'] ) || !isset( $post['zip'] ) || !isset( $post['country'] ) || !isset( $post['state'] ) || !isset( $post['city'] ) || !isset( $post['mapmarker'] ) || !\query\main::have_store( $post['store'], $GLOBALS['me']->ID ) ) {
        ...
        $stmt = $db->stmt_init();
        $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "store_locations (user, store, country, countryID, state, stateID, city, cityID, zip, address, lat, lng, lastupdate_by, lastupdate, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())" );
        ...
        $stmt->bind_param( "iisisisissddi", $GLOBALS['me']->ID, $post['store'], $country_name, $country_id, $state_name, $state_id, $city_name, $city_id, $post['zip'], $post['address'], $post['mapmarker'][0], $post['mapmarker'][1], $GLOBALS['me']->ID );
        $execute = $stmt->execute();
        $stmt->close();
...
public static function edit_store_location( $id, $post ) {
    ...
    if( !isset( $post['store'] ) || !isset( $post['address'] ) || !isset( $post['zip'] ) || !isset( $post['country'] ) || !isset( $post['state'] ) || !isset( $post['city'] ) || !isset( $post['mapmarker'] ) || !\query\main::have_store( $post['store'], $GLOBALS['me']->ID ) ) {
        ...
        $stmt = $db->stmt_init();
        $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "store_locations SET store = ?, country = ?, countryID = ?, state = ?, stateID = ?, city = ?, cityID = ?, zip = ?, address = ?, lat = ?, lng = ?, lastupdate_by = ?, lastupdate = NOW() WHERE id = ?" );
        ...
        $stmt->bind_param( "isisisissddii", $post['store'], $country->name, $post['country'], $state->name, $post['state'], $city->name, $post['city'], $post['zip'], $post['address'], $post['mapmarker'][0], $post['mapmarker'][1], $GLOBALS['me']->ID, $id );
        $execute = $stmt->execute();
        $stmt->close();
...
public static function check_coupon_code( $code ) {
    ...
    if( !$GLOBALS['me'] || !isset( $code ) || !( $coupon_id = \query\claims::coupon_code_exists( $code ) ) ) {
        throw new \Exception( t( 'msg_error', "Error!" ) );
    } else {

        $stmt = $db->stmt_init();
        $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "coupon_claims SET used = 1, used_date = NOW() WHERE id = ?" );
...
public static function set_coupon_code_unused( $code ) {
    ...
    if( !$GLOBALS['me'] || !( $coupon_id = \query\claims::coupon_code_exists( $code ) ) ) {
        throw new \Exception( t( 'msg_error', "Error!" ) );
    } else {
