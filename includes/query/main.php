...
class main
{
    ...
    public static function user_plugins($scope = '', $view = '')
    {
        ...
        $data = array();
        while ($stmt->fetch()) {
            $data[] = (object) array('ID' => $id, 'user' => $user, 'name' => esc_html($name), 'image' => esc_html($image), 'scope' => esc_html($scope), 'main_file' => esc_html($main), 'load_file' => esc_html($loader), 'menu_icon' => $menu_icon, 'subadmin_view' => (bool) $subadmin_view, 'vars' => @unserialize($vars), 'in_menu' => (bool) $menu, 'is_active' => ($visible !== 0 ? true : false), 'date' => $date);
        }
    ...
    public static function category_info($id = 0, $special = array())
    {
        ...
        // special
        $useem = (isset($special['no_emoticons']) && $special['no_emoticons']) ? false : (bool) \query\main::get_option('smilies_categories');
    ...
    public static function widget_info($id = 0)
    {
        ...
        return (object) array('id' => $id, 'user' => $user, 'widget_id' => $widget, 'zone' => esc_html($sidebar), 'title' => esc_html($title), 'limit' => $stop, 'type' => $type, 'orderby' => $orderby, 'position' => $position, 'text' => esc_html($text), 'extra' => @unserialize($extra), 'html' => (bool) $html, 'mobile_view' => (bool) $mobile_view, 'date' => $date);
    }
    ...
    public static function show_widgets($id)
    {
        ...
        while ($stmt->fetch()) {
            if (file_exists(DIR . '/' . $location)) {
                $data[] = array('ID' => $id, 'widget_id' => $widget, 'title' => esc_html($title), 'limit' => $limit, 'type' => $type, 'orderby' => $orderby, 'content' => \site\content::content('content', $text, true, true, false, !$html), 'extra' => @unserialize($extra), 'mobile_view' => (bool) $mobile_view, 'file' => $location);
            }
        }
    ...
    public static function user_info($user_id = 0, $special = array())
    {
        ...
        return (object) value_with_filter('user_info_values', array('ID' => $id, 'name' => \site\content::title('user_name_single', $name, $useem, $useec, $usefl), 'email' => esc_html($email), 'avatar' => esc_html($avatar), 'points' => $points, 'credits' => $credits, 'IP' => esc_html($ip), 'privileges' => $privileges, 'erole' => @unserialize($erole), 'is_subscribed' => (bool) $subscriber, 'is_confirmed' => (bool) $valid, 'is_banned' => (strtotime($ban) > time() ? true : false), 'is_subadmin' => ($privileges === 1 ? true : false), 'is_admin' => ($privileges > 1 ? true : false), 'last_login' => $last_login, 'last_action' => $last_action, 'visits' => $visits, 'ban' => $ban, 'refid' => $refid, 'extra' => @unserialize($extra), 'date' => $date, 'reviews' => $reviews, 'stores' => $stores, 'coupons' => $coupons, 'products' => $products));
    }
    ...
    public static function product_info($product_id = 0, $special = array())
    {
        ...
        // special
        $useem = (isset($special['no_emoticons']) && $special['no_emoticons']) ? false : (bool) \query\main::get_option('smilies_products');
        $usesh = (isset($special['no_shortcodes']) && $special['no_shortcodes']) ? false : true;
        $useec = (isset($special['no_escape']) && $special['no_escape']) ? false : true;
        $usefl = (isset($special['no_filters']) && $special['no_filters']) ? false : true;

        return (object) value_with_filter('product_info_values', array('ID' => $id, 'feedID' => $feed_id, 'userID' => $user, 'storeID' => $store, 'catID' => $cat, 'user_name' => esc_html($user_name), 'title' => \site\content::title('product_name_single', $title, $useem, $useec, $usefl), 'original_url' => $link, 'url' => (preg_match('/^http(s)?/i', $link) ? esc_html($link) : esc_html($store_link)), 'description' => \site\content::content('product_single', $description, $useem, $usesh, false, $useec, $usefl), 'tags' => esc_html($tags), 'image' => esc_html($image), 'price' => $price, 'old_price' => ($old_price > 0 && $old_price > $price ? $old_price : 0), 'currency' => esc_html($currency), 'visible' => (bool) $visible, 'views' => $views, 'start_date' => $start, 'is_running' => (strtotime($start) < time() && strtotime($expiration) > time() ? true : false), 'expiration_date' => $expiration, 'cashback' => $cashback, 'is_available_online' => (!(bool) $store_type || $store_sellonline ? true : false), 'meta_title' => esc_html($meta_title), 'meta_description' => esc_html($meta_desc), 'meta_keywords' => esc_html($meta_keywords), 'lastupdate_by' => $lastupdate_by, 'lastupdate_by_name' => esc_html($lastupdate_by_name), 'last_update' => $last_update, 'is_started' => (strtotime($start) > time() ? false : true), 'is_expired' => (strtotime($expiration) > time() ? false : true), 'is_popular' => (bool) $popular, 'paid_until' => $paid_until, 'extra' => @unserialize($extra), 'date' => $date, 'store_is_physical' => (bool) $store_type, 'store_img' => esc_html($store_img), 'storeID' => $store_id, 'store_catID' => $store_cat, 'store_name' => esc_html($store_name), 'store_url' => esc_html($store_link), 'reviews' => $reviews, 'stars' => $stars, 'store_sellonline' => (bool) $store_sellonline, 'url_title' => esc_html($url_title), 'link' => ($seo_link ? \site\utils::make_seo_link(\query\main::get_option('seo_link_product'), $title, $url_title, $id, $extension) : $GLOBALS['siteURL'] . '?product=' . $id), 'store_link' => ($seo_link ? \site\utils::make_seo_link(\query\main::get_option('seo_link_store'), $store_name, $store_url_title, $store_id, $extension) : $GLOBALS['siteURL'] . '?store=' . $store_id), 'store_reviews_link' => ($seo_link ? \site\utils::make_seo_link(\query\main::get_option('seo_link_reviews'), $store_name, $store_url_title, $store_id, $extension) : $GLOBALS['siteURL'] . '?reviews=' . $store_id)));
    }
    ...
    public static function store_info($store_id = 0, $special = array())
    {
        ...
        $stmt->prepare("SELECT s.id, s.feedID, s.user, s.category, s.popular, s.physical, s.name, s.link, s.description, s.tags, s.image, s.hours, s.phoneno, s.sellonline, s.visible, s.views, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "coupons WHERE store = s.id AND visible > 0), s.url_title, s.meta_title, s.meta_keywords, s.meta_desc, s.lastupdate_by, (SELECT name FROM " . DB_TABLE_PREFIX . "users WHERE id = s.lastupdate_by), s.lastupdate, s.extra, s.date, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "reviews WHERE store = s.id AND valid > 0), (SELECT AVG(stars) FROM " . DB_TABLE_PREFIX . "reviews WHERE store = s.id AND valid > 0), (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "products WHERE store = s.id AND visible > 0), u.name, s.network, n.name as network_name FROM " . DB_TABLE_PREFIX . "stores s LEFT JOIN " . DB_TABLE_PREFIX . "users u ON (u.id = s.user) LEFT JOIN " . DB_TABLE_PREFIX . "network n ON (n.id = s.network) WHERE s.id = ? OR s.url_title = ?");
        $stmt->bind_param("is", $store_id, $store_id);
        $stmt->execute();
        $stmt->bind_result($id, $feed_id, $user, $cat, $popular, $physical, $name, $link, $description, $tags, $image, $hours, $phone, $sellonline, $visible, $views, $coupons, $url_title, $meta_title, $meta_keywords, $meta_desc, $lastupdate_by, $lastupdate_by_name, $last_update, $extra, $date, $reviews, $stars, $products, $user_name, $network, $network_name);
        $stmt->fetch();
        $stmt->close();
    ...
    public static function while_network($network = array(), $special = array())
    {
     ...
     (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "stores WHERE network = c.id) FROM " . DB_TABLE_PREFIX . "network c"
        ...
        while ($stmt->fetch()) {
            ...
            $data[] = (object) array(
                'ID' => $id, 'subcatID' => $subcategory, 'user' => $user,
    ...
    public static function while_users($category = array(), $special = array())
    {
        ...
        while ($stmt->fetch()) {
            ...
            $data[] = (object) value_with_filter('user_info_values', array('ID' => $id, 'name' => \site\content::title('users_name_list', $name, $useem, $useec, $usefl), 'email' => esc_html($email), 'avatar' => esc_html($avatar), 'points' => $points, 'credits' => $credits, 'IP' => esc_html($ip), 'privileges' => $privileges, 'is_subscribed' => (bool) $subscriber, 'is_confirmed' => (bool) $valid, 'is_banned' => (strtotime($ban) > time() ? true : false), 'is_subadmin' => ($privileges === 1 ? true : false), 'is_admin' => ($privileges > 1 ? true : false), 'last_login' => $last_login, 'last_action' => $last_action, 'visits' => $visits, 'ban' => $ban, 'refid' => $refid, 'extra' => @unserialize($extra), 'date' => $date));
        }
    ...
    public static function have_items($category = array(), $place = '', $special = array())
    {
        ...
        if (isset($categories['show'])) {
            ...
            $custom_where_clause = value_with_filter('items_where_clause', array(
                'expired'   => 'c.expiration <= NOW()',
                'active'    => 'c.expiration > NOW()',
                'popular'   => 'c.popular > 0',
        ...
        switch ($place) {
            ...
            case 'category':
                ...
                while ($stmt->fetch()) {
                    ...
                    $data[] = (object) value_with_filter('coupon_info_values', array('ID' => $id, 'feedID' => $feed_id, 'userID' => $user, 'storeID' => $store, 'catID' => $cat, 'code' => esc_html($code), 'title' => \site\content::title('items_name_list', $title, $useem, $useec, $usefl), 'url' => (preg_match('/^http(s)?/i', $link) ? esc_html($link) : esc_html($store_link)), 'original_url' => $link, 'description' => \site\content::content('items_list', $description, $useem, $usesh, false, $useec, $usefl), 'tags' => esc_html($tags), 'image' => esc_html($image), 'image2' => (!empty($image) ? (!filter_var($image, FILTER_VALIDATE_URL) ? $GLOBALS['siteURL'] . esc_html($image) : esc_html($image)) : ''), 'source' => (empty($source) ? '' : (filter_var($source, FILTER_VALIDATE_URL) ? esc_html($source) : $GLOBALS['siteURL'] . esc_html($source))), 'visible' => (bool) $visible, 'views' => $views, 'clicks' => $clicks, 'start_date' => $start, 'is_coupon' => (!empty($code) ? true : false), 'is_deal' => (empty($code) ? true : false), 'is_running' => (strtotime($start) < time() && strtotime($expiration) > time() ? true : false), 'expiration_date' => $expiration, 'cashback' => $cashback, 'is_printable' => ((bool) $store_type ? (bool) $printable : false), 'is_show_in_store' => ((bool) $store_type ? (bool) $show_in_store : false), 'is_available_online' => (!(bool) $store_type || (bool) $avab_online ? true : false), 'is_local_source' => (empty($source) || filter_var($source, FILTER_VALIDATE_URL) ? false : true), 'is_started' => (strtotime($start) > time() ? false : true), 'is_expired' => (strtotime($expiration) > time() ? false : true), 'is_popular' => (bool) $popular, 'is_exclusive' => (bool) $exclusive, 'is_verified' => (bool) $verified, 'last_check' => $last_verif, 'votes' => $votes, 'votes_percent' => $votes_percent, 'claim_limit' => $claim_limit, 'claims' => $claims, 'paid_until' => $paid_until, 'extra' => @unserialize($extra), 'date' => $date, 'reviews' => $reviews, 'stars' => $stars, 'store_is_physical' => (bool) $store_type, 'store_img' => esc_html($store_img), 'store_catID' => $store_cat, 'store_name' => esc_html($store_name), 'store_url' => esc_html($store_link), 'store_phone_no' => esc_html($store_phone), 'store_sellonline' => (bool) $store_sellonline, 'url_title' => esc_html($url_title), 'link' => ($seo_link ? \site\utils::make_seo_link($seo_link_coupon, $title, $url_title, $id, $extension) : $GLOBALS['siteURL'] . '?id=' . $id), 'store_link' => ($seo_link ? \site\utils::make_seo_link($seo_link_store, $store_name, $store_url_title, $store, $extension) : $GLOBALS['siteURL'] . '?store=' . $store), 'store_reviews_link' => ($seo_link ? \site\utils::make_seo_link($seo_link_reviews, $store_name, $store_url_title, $store, $extension) : $GLOBALS['siteURL'] . '?reviews=' . $store)));
                }
    ...
    public static function have_products($category = array(), $place = '', $special = array())
    {
        ...
        switch ($place) {
            ...
            case 'search':
                ...
                $stmt->prepare("SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "products p LEFT JOIN " . DB_TABLE_PREFIX . "stores s ON (s.id = p.store) WHERE (MATCH(p.title) AGAINST (? IN BOOLEAN MODE) OR MATCH(p.tags) AGAINST (? IN BOOLEAN MODE) OR MATCH(s.name) AGAINST (? IN BOOLEAN MODE) OR MATCH(s.tags) AGAINST (? IN BOOLEAN MODE))" . (empty($where) ? '' : ' AND ' . implode(' AND ', array_filter($where))));
        ...
        switch ($place) {
            ...
            case 'category':
                ...
                while ($stmt->fetch()) {
                    ...
                    $data[] = (object) value_with_filter('product_info_values', array('ID' => $id, 'feedID' => $feed_id, 'userID' => $user, 'storeID' => $store, 'catID' => $cat, 'title' => \site\content::title('products_name_list', $title, $useem, $useec, $usefl), 'url' => (preg_match('/^http(s)?/i', $link) ? esc_html($link) : esc_html($store_link)), 'original_url' => $link, 'description' => \site\content::content('products_list', $description, $useem, $usesh, false, $useec, $usefl), 'tags' => esc_html($tags), 'image' => esc_html($image), 'price' => $price, 'old_price' => ($old_price > 0 && $old_price > $price ? $old_price : 0), 'currency' => esc_html($currency), 'visible' => (bool) $visible, 'views' => $views, 'start_date' => $start, 'is_running' => (strtotime($start) < time() && strtotime($expiration) > time() ? true : false), 'expiration_date' => $expiration, 'cashback' => $cashback, 'is_available_online' => (!(bool) $store_type || $store_sellonline ? true : false), 'is_started' => (strtotime($start) > time() ? false : true), 'is_expired' => (strtotime($expiration) > time() ? false : true), 'paid_until' => $paid_until, 'extra' => @unserialize($extra), 'date' => $date, 'reviews' => $reviews, 'stars' => $stars, 'store_is_physical' => (bool) $store_type, 'store_img' => esc_html($store_img), 'storeID' => $store_id, 'store_catID' => $store_cat, 'store_name' => esc_html($store_name), 'store_url' => esc_html($store_link), 'store_sellonline' => (bool) $store_sellonline, 'url_title' => esc_html($url_title), 'link' => ($seo_link ? \site\utils::make_seo_link($seo_link_product, $title, $url_title, $id, $extension) : $GLOBALS['siteURL'] . '?product=' . $id), 'store_link' => ($seo_link ? \site\utils::make_seo_link($seo_link_store, $store_name, $store_url_title, $store_id, $extension) : $GLOBALS['siteURL'] . '?store=' . $store_id), 'store_reviews_link' => ($seo_link ? \site\utils::make_seo_link($seo_link_reviews, $store_name, $store_url_title, $store_id, $extension) : $GLOBALS['siteURL'] . '?reviews=' . $store_id)));
                }
    ...
    public static function while_reviews($category = array(), $place = '', $special = array())
    {
        ...
        switch ($place) {
            ...
            case 'store':
                ...
                $data = array();
                while ($stmt->fetch()) {
                    $data[] = (object) array('ID' => $id, 'userID' => $user, 'storeID' => $store, 'user_name' => esc_html($user_name), 'text' => \site\content::content('reviews_list', $text, $useem, $usesh, false, $useec, $usefl), 'stars' => $stars, 'valid' => (bool) $valid, 'date' => $date, 'user_avatar' => esc_html($user_avatar));
                }
    ...
    public static function have_stores($category = array(), $place = '', $special = array())
    {
        ...
        switch ($place) {
            ...
            case 'category':
                ...
                $stmt->prepare("SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "stores WHERE category IN(" . implode(',', $ids) . ") AND visible > 0" . (empty($where) ? '' : ' AND ' . implode(' AND ', array_filter($where))));
                $stmt->execute();
                $stmt->bind_result($count);
                $stmt->fetch();
            ...
            case 'search':
                ...
                $stmt->prepare("SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "stores WHERE (MATCH(name) AGAINST (? IN BOOLEAN MODE) OR MATCH(tags) AGAINST (? IN BOOLEAN MODE)) AND visible > 0" . (empty($where) ? '' : ' AND ' . implode(' AND ', array_filter($where))));
            ...
            default:
                ...
                $stmt = $db->stmt_init();
                $stmt->prepare("SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "stores" . (empty($where) ? '' : ' WHERE ' . implode(' AND ', array_filter($where))));
                $stmt->execute();
                $stmt->bind_result($count);
                $stmt->fetch();
    ...
    public static function while_rewards($category = array())
    {
        ...
        $data = array();
        while ($stmt->fetch()) {
            $data[] = (object) array('ID' => $id, 'user' => $user, 'points' => $points, 'title' => esc_html($title), 'description' => esc_html($description), 'image' => esc_html($image), 'fields' => @unserialize($fields), 'visible' => (bool) $visible, 'date' => $date);
        }
    ...
    */
        ...
        $data = array();
        while ($stmt->fetch()) {
            $data[] = (object) array('ID' => $id, 'name' => esc_html($name), 'user' => $user, 'points' => $points, 'reward' => $reward, 'reward_exists' => ($reward_exists > 0 ? true : false), 'fields' => @unserialize($fields), 'claimed' => (bool) $claimed, 'date' => $date);
        }
