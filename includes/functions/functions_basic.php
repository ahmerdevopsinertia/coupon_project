...
function value_with_filter($filter = '', $default = '', $extra = '')
{
    ...
    if ($filter == 'meta-description' || $filter == 'site-title-tag') {
        ...
        for ($i = 0; $i <= count($stringArray) - 1; $i++) { 
            $defaultArray[$i] = $stringArray[$i];
            if ($stringArray[$i] == '[fmonth],') {
                $month = date('M');
                $defaultArray[$i] = $month . ',';   
            }

            if ($stringArray[$i] == '[year]:') {
                $year = date('Y');
                $defaultArray[$i] = $year. '.';
            }
...
{
    return (bool) \query\main::get_option('maintenance');
}
...
function read_template_part($part)
{
    ...
    switch ($part) {
        case '404':
            include($theme_location . '/404.php');
            break;
    }
}
...
function price_format($price, $currency = '')
{
    return (!empty($currency) ?
        value_with_filter(
            'price_format_currency_position',
            sprintf('%s %s', \site\utils::money_format($price), $currency),
            array(
...
function store_price($format = false)
{
    ...
    return ($format ? price_format($price) : (float) $price);
}
