...
function have_items_custom( $category = array(), $special = array() ) {
    return \query\main::have_items( $category, '', $special );
}

/* SHOW CUSTOM ITEMS */

function items_custom( $category = array(), $special = array() ) {
    return \query\main::while_items( $category, '', $special );
}
...
function have_products_custom( $category = array(), $special = array() ) {
    return \query\main::have_products( $category, '', $special );
}

/* SHOW CUSTOM PRODUCTS */

function products_custom( $category = array(), $special = array() ) {
    return \query\main::while_products( $category, '', $special );
}
...
function have_stores_custom( $category = array(), $special = array() ) {
    return \query\main::have_stores( $category, '', $special );
}

/* SHOW CUSTOM STORES */

function stores_custom( $category = array(), $special = array() ) {
...
function login_form( $redirect_url = '', $loc = 'regular' ) {
    ...
    try {
        ...
        $form .= '<meta http-equiv="refresh" content="2; url='. $GLOBALS['siteURL'] . 'setSession.php' . ( !empty( $redirect_url ) ? '?back=' . $redirect_url : '' ) . '">';
