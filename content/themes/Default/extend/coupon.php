...
function couponscms_coupon_item( $item = object, $owner_view = false, $single_redirect = false ) {
    ...
    if( $item->is_printable ) {
        ...
        if( couponscms_view_store_coupons( $item->storeID ) ) {
            ...
            $markup .= '<a href="' . get_target_link( 'coupon', $item->ID, array( 'reveal_code' => true, 'backTo' => ( $single_redirect ? $item->link : get_update() ) ) ) . '" target="_blank" class="icon-button icon-border" data-target-on-click="' . get_target_link( 'coupon', $item->ID ) . '">' . $code_preview . '<span>' . t( 'theme_claimed_show_code', 'View Code' ) . '</span></a>';
        }
    } else {
