...
public static function add_store_location( $opt = array() ) {
    ...
    $stmt->bind_param( "iisisisissddi", $GLOBALS['me']->ID, $opt['Store'], $country_name, $country_id, $state_name, $state_id, $city_name, $city_id, $opt['Zip'], $opt['Address'], $lat, $lng, $GLOBALS['me']->ID );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function edit_store_location( $id, $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "store_locations SET country = ?, countryID = ?, state = ?, stateID = ?, city = ?, cityID = ?, zip = ?, address = ?, lat = ?, lng = ?, lastupdate_by = ?, lastupdate = NOW() WHERE id = ?" );
    ...
    $stmt->bind_param( "sisisissddii", $country->name, $opt['Country'], $state->name, $opt['State'], $city->name, $opt['City'], $opt['Zip'], $opt['Address'], $lat, $lng, $GLOBALS['me']->ID, $id );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function edit_user( $id, $opt = array() ) {
    ...
    $stmt->bind_param( "sssiiisiisi", $opt['name'], $opt['email'], $avatar, $opt['points'], $opt['credits'], $opt['privileges'], @serialize( $opt['erole'] ), $opt['subscriber'], $opt['confirm'], $extra, $id );
    $execute = $stmt->execute();
    $stmt->close();
