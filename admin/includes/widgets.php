...
public static function build_extra( $fields = array(), $value = array(), $extra_id = '', $post_name = 'extra', $item = false ) {
    ...
    foreach( $fields as $id => $opt ) {
        ...
        switch( $opt['type'] ) {
            ...
            case 'checkbox':
                $markup .= '<div class="row' . ( !empty( $class ) ? ' ' . implode( $class ) : '' ) . '"' . ( !empty( $opt['row_atts'] ) ? \site\utils::build_atts( $opt['row_atts'] ) : '' ) . '><span>' . esc_html( $opt['title'] ) . ( !empty( $opt['info'] ) ? ' <span class="info"><span>' . esc_html( $opt['info'] ) . '</span></span>' : '' ) . ':</span><div><input name="' . $post_name . $extra_id . '[' . $id . ']" id="' . $post_name . $extra_id . '[' . $id . ']"' . ( !empty( $opt['atts'] ) ? \site\utils::build_atts( $opt['atts'] ) : '' ) . ' type="checkbox"' . ( !empty( $value[$id] ) ? ' checked' : ( !empty( $opt['default'] ) ? ' checked' : '' ) ) . '> <label for="' . $post_name . $extra_id . '[' . $id . ']"><span></span> ' . ( !empty( $opt['label'] ) ? esc_html( $opt['label'] ) : '' ) . '</label></div></div>';
            break;
