...
if( $_SERVER['REQUEST_METHOD'] == 'POST' && isset( $_POST['csrf'] ) && check_csrf( $_POST['csrf'], 'settings_csrf' ) ) {
    ...
    'price_max_days' => $_POST['max_days'],
    'price_product' => $_POST['product'],
    'price_product_max_days' => $_POST['max_days_p']
    ) ) )
...
<div class="row"><span>' . t( 'settings_from_priccoupon', "Coupon (credits)" ) . ' <span class="info"><span>' . t( 'settings_from_ipriccoupon', "The price for every coupon added. If expiration date isn't greater than maximum days." ) . '</span></span>:</span><div><input type="number" name="coupon" value="' . (int) \query\main::get_option( 'price_coupon' ) . '" min="0" /></div></div>
<div class="row"><span>' . t( 'settings_from_priccouponmd', "Coupon (max. days)" ) . ' <span class="info"><span>' . t( 'settings_from_ipriccouponmd', "Ex.: If you set this to 10 (days) and the price for every coupon (1 credit), an user will pay 1 credit for every 10 days. For a coupon with expiration date after 56 days, the user will pay 6 credits." ) . '</span></span>:</span><div><input type="number" name="max_days" value="' . (int) \query\main::get_option( 'price_max_days' ) . '" min="1" /></div></div>
<div class="row"><span>' . t( 'settings_from_pricproduct', "Product (credits)" ) . ' <span class="info"><span>' . t( 'settings_from_ipricproduct', "The price for every product added. If expiration date isn't greater than maximum days." ) . '</span></span>:</span><div><input type="number" name="product" value="' . (int) \query\main::get_option( 'price_product' ) . '" min="0" /></div></div>
<div class="row"><span>' . t( 'settings_from_pricproductmd', "Product (max. days)" ) . ' <span class="info"><span>' . t( 'settings_from_ipricproductmd', "Ex.: If you set this to 10 (days) and the price for every product (1 credit), an user will pay 1 credit for every 10 days. For a product with expiration date after 56 days, the user will pay 6 credits." ) . '</span></span>:</span><div><input type="number" name="max_days_p" value="' . (int) \query\main::get_option( 'price_product_max_days' ) . '" min="1" /></div></div>
