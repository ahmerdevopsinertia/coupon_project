...
public static function coupon_code_exists( $code ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare("SELECT cc.id FROM " . DB_TABLE_PREFIX . "coupon_claims cc LEFT JOIN " . DB_TABLE_PREFIX . "coupons c ON (c.id = cc.coupon) LEFT JOIN " . DB_TABLE_PREFIX . "stores s ON (s.id = c.store) WHERE cc.code = ? AND s.user = ?" );
